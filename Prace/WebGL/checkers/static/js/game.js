import { getLastMove, getMap, move } from "./api.js";
...
export default class GameManager {
			...
			if (intersects[0].object instanceof Pawn) {
				if (intersects[0].object.isYours) {
					if (this.selected) {
						...
						this.getBoard().unhighlightMoves();
					}
					this.selected = intersects[0].object;
					this.selected.onClick();
					this.getBoard().highlightMoves(this.selected);
	...
	render() {
		...
		if (GameManager.getState("isYourTurn") !== this.lastTurn) {
			...
			setTimeout(async () => {
				let res = await getLastMove();
				if (res.sucess) {
					this.getBoard().useMove(res.lastMove);
				}
			}, 200);
		}
		if (GameManager.getState("isEnd")) {
			setTimeout(async () => {
	...
	getBoard() {
		return this.scene.children
			.filter((ele) => ele instanceof Scene)[0]
			.children.filter((ele) => ele instanceof Board)[0];
	}
}
